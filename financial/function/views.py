...
class IndexView(ListView):
    model = Function
    template_name = 'function/index.html'
    context_object_name = 'data_list'

    def get_queryset(self):
        return Function.objects.all().filter(isdeleted=0).order_by('-pk')
...
class CreateView(CreateView):
    model = Function
    template_name = 'function/create.html'
    fields = ['code', 'description']

    def dispatch(self, request, *args, **kwargs):
        if not request.user.has_perm('function.add_function'):
            raise Http404
